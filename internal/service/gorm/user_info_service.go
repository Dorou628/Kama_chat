...
func (u *userInfoService) SmsLogin(req request.SmsLoginRequest) (string, *respond.LoginRespond, int) {
	...
	// 使用统一的验证码校验接口
	isValid, message := sms.VerifyCode(req.Telephone, req.SmsCode)
	if !isValid {
		zlog.Info("短信验证码校验失败: " + message)
		return message, nil, -2
	}
...
func (u *userInfoService) Register(registerReq request.RegisterRequest) (string, *respond.RegisterRespond, int) {
	// 使用统一的验证码校验接口
	isValid, message := sms.VerifyCode(registerReq.Telephone, registerReq.SmsCode)
	if !isValid {
		zlog.Info("注册验证码校验失败: " + message)
		return message, nil, -2
	}
	// 不用校验手机号，前端校验
	// 判断电话是否已经被注册过了
