...
export default {
  ...
  setup() {
    ...
    const sendFileMessage = async (fileUrl, actualFileName) => {
      const chatFileMessageRequest = {
        ...
        send_avatar: data.userInfo.avatar,
        receive_id: data.contactInfo.contact_id,
        file_size: getFileSize(data.fileList[0] ? data.fileList[0].size : 0),
        file_name: actualFileName || (data.fileList[0] ? data.fileList[0].name : ""),
        file_type: data.fileList[0] ? data.fileList[0].type : "",
      };
    ...
    const handleUploadSuccess = (response, file) => {
      ...
      // 使用实际上传的文件名，而不是前端的文件名
      const actualFileName = file.raw.name;
      console.log("实际文件名:", actualFileName);
      sendFileMessage(
        store.state.backendUrl + "/static/files/" + actualFileName,
        actualFileName
      );
      data.fileList = [];
    };
    ...
    const downloadFile = async (fileName) => {
      try {
        ...
        // 使用新的下载接口
        const downloadUrl = store.state.backendUrl + "/download/file/" + encodedFileName;
        ...
        console.log("下载响应:", rsp);
        console.log("响应状态:", rsp.status);
        console.log("响应头:", rsp.headers);
        
        if (rsp.status !== 200) {
          console.error("下载失败，状态码:", rsp.status);
          ElMessage.error("文件下载失败，状态码: " + rsp.status);
          return;
        }
        ...
        if (error.response) {
          ...
          ElMessage.error("文件下载失败: " + error.response.status + " - " + (error.response.statusText || "未知错误"));
        } else {
